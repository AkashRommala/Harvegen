...
function About() {
  ...
  return (
    <>
      {/* Page Hero */}
      <header className="pt-[120px] pb-15 relative overflow-hidden bg-gradient-to-br from-primary-50 via-primary-50 to-primary-50">
        <div className="absolute inset-0 bg-gradient-to-r from-primary-500/5 via-primary-600/5 to-transparent rounded-full blur-3xl animate-gradient"></div>
        <div className="absolute w-[600px] h-[400px] rounded-full bg-gradient-to-r from-primary-500/20 to-primary-600/20 top-0 left-1/2 -translate-x-1/2 pointer-events-none animate-float"></div>
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-[0.72rem] text-slate-500 mb-4 font-mono">
            <Link href="/" className="text-slate-400 hover:text-primary-600">index</Link> <span>/ about</span>
          </div>
          <div className="inline-flex items-center gap-2 mb-3 font-mono text-[0.7rem] tracking-widest uppercase text-primary-600">
            <span className="w-6 h-px bg-primary-600"></span>
            Our Story
          </div>
              <h1 className="mb-3 text-slate-900 font-display">Built for Engineers,<br/><span className="text-primary-600">By Engineers</span></h1>
        ...
        <section className="py-20">
          <div className="max-w-[1160px] mx-auto px-6">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-9 mb-5">
              <blockquote className="font-mono text-clamp text-slate-100 leading-relaxed border-l-2 border-primary-600 pl-5">
            ...
            <div className="grid md:grid-cols-2 gap-10 items-start">
              <div>
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  <Button href="/tutorials" variant="outline" size="default" icon="bookOpen">Start Learning</Button>
                </div>
              </div>
              <div className="bg-slate-800 border border-slate-700 rounded-xl h-80 flex items-center justify-center text-7xl relative overflow-hidden">
                ...
                <span className="relative z-10"><FiZap className="w-16 h-16 text-primary-600" /></span>
              </div>
            </div>
          </div>
        </section>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {[{ name: 'Arjun Reddy', role: 'Founder & Lead Developer', bio: '5+ years firmware development. Former automotive ECU developer passionate about bare-metal programming and RTOS.', initials: 'AR' },
                ...
                <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-6 text-center">
                  <div className="w-16 h-16 bg-gradient-to-br from-primary-500 to-primary-600 rounded-full mx-auto mb-4 flex items-center justify-center font-mono text-xl font-semibold text-white">{member.initials}</div>
                  <h4 className="font-mono text-base font-semibold text-slate-100 mb-1">{member.name}</h4>
                  <div className="text-primary-600 text-[0.75rem] mb-3">{member.role}</div>
            ...
            <div className="grid md:grid-cols-5 gap-12">
              <div className="md:col-span-2">
                ...
                <div className="mb-6">
                  {[{ icon: FiMail, label: 'Email', val: 'hello@ephub.dev' },
                    ...
                    <div key={i} className="flex items-center gap-3.5 py-3.5 border-b border-slate-700">
                      <div className="w-9 h-9 bg-primary-500/10 border border-primary-500/25 rounded flex items-center justify-center text-lg shrink-0"><item.icon className="w-4 h-4 text-primary-600" /></div>
                      <div>
                        <div className="font-mono text-[0.65rem] tracking-widest uppercase text-slate-500 mb-0.5">{item.label}</div>
                        <div className="font-mono text-[0.82rem] text-primary-600">{item.val}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex gap-2">
                  <a href="#" className="w-9 h-9 bg-slate-700 border border-slate-600 rounded flex items-center justify-center text-lg hover:border-primary-500 hover:bg-primary-500/10 transition-all"><FiGithub /></a>
                  <a href="#" className="w-9 h-9 bg-slate-700 border border-slate-600 rounded flex items-center justify-center text-lg hover:border-primary-500 hover:bg-primary-500/10 transition-all"><FaYoutube /></a>
                  <a href="#" className="w-9 h-9 bg-slate-700 border border-slate-600 rounded flex items-center justify-center text-lg hover:border-primary-500 hover:bg-primary-500/10 transition-all"><FiTwitter /></a>
                  <a href="#" className="w-9 h-9 bg-slate-700 border border-slate-600 rounded flex items-center justify-center text-lg hover:border-primary-500 hover:bg-primary-500/10 transition-all"><FiLinkedin /></a>
                </div>
              </div>
              ...
              <form className="md:col-span-3 bg-slate-800 border border-slate-700 rounded-xl p-7" onSubmit={handleSubmit}>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input type="text" placeholder="Your name" required className="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-700 rounded text-[0.85rem] text-slate-100 font-mono focus:outline-none focus:border-primary-500 transition-colors" />
                  </div>
                  <div>
                    <label className="block font-mono text-[0.72rem] text-slate-400 mb-1.5">Email</label>
                    <input type="email" placeholder="you@example.com" required className="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-700 rounded text-[0.85rem] text-slate-100 font-mono focus:outline-none focus:border-primary-500 transition-colors" />
                  </div>
                </div>
                <div className="mb-4">
                  <label className="block font-mono text-[0.72rem] text-slate-400 mb-1.5">Subject</label>
                  <input type="text" placeholder="Project contribution / Bug report / Collaboration" className="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-700 rounded text-[0.85rem] text-slate-100 font-mono focus:outline-none focus:border-primary-500 transition-colors" />
                </div>
                <div className="mb-4">
                  <label className="block font-mono text-[0.72rem] text-slate-400 mb-1.5">Message</label>
                  <textarea rows="5" placeholder="Tell us what's on your mind…" required className="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-700 rounded text-[0.85rem] text-slate-100 font-mono focus:outline-none focus:border-primary-500 transition-colors resize-none"></textarea>
                </div>
                <Button type="submit" variant="default" size="default" icon="send">Send Message</Button>
              </form>
            </div>
          </div>
        </section>
